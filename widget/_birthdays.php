<?php
$today = getTodayDate();
$birthdays = Athlete::getLastTen('', '', $today, '');
$labelDay = ucwords($today ?? '');
if ($birthdays) {
    ?>
    <div class="container">
        <div class="row mb-5">
            <?= getTitle(T_TITLE_BIRTHDAY . ' ' . $labelDay) ?>
                ...
                foreach ($birthdays as $ba) {
                    ...
                    <div class="d-inline-flex align-items-top">
                        <a class="link-light text-decoration-none me-2" aria-current="page"
                           href="<?= getLinkAtleta($ba->id, $baTitle) ?>">
                            ...
                            >
                            <span class="d-block mt-1 text-wrap text-center"><?= $baSurname ?></span>
                        </a>
                    </div>
                    <?php
                }
