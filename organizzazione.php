...
if (array_key_exists('description', $_GET)) {
    ...
    $fed = Organizations::getByDescription($description);
    if ($fed) {
        $descriptionFed = htmlspecialchars($fed->description);
        $title = htmlspecialchars($description);
        $news_filter = htmlspecialchars($fed->sport);
        $obja = new Athlete();
        $obja->title = $description;
        $obja->sport = $fed->sport;
        $website = generateLinkSocial($fed->website ?? '', 'www');
        $facebook = generateLinkSocial($fed->facebook ?? '', 'fb');
        $instagram = generateLinkSocial($fed->instagram ?? '', 'ig');
        $tiktok = generateLinkSocial($fed->tiktok ?? '', 'tiktok');
        $twitter = generateLinkSocial($fed->twitter ?? '', 'x');
        $store = generateLinkSocial($fed->store ?? '', 'store');
        $youtube = generateLinkSocial($fed->youtube ?? '', 'yt');
                ...
                <div class="col-12">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 col-md-10">
                            <?= getTitle($descriptionFed, 'h3') ?>
                            <div class="row" style="margin-top: -13px;">
                                <div class="col-12">
                                    <?= $website ?> <?= $store ?> <?= $youtube ?> <?= $instagram ?> <?= $facebook ?> <?= $tiktok ?> <?= $twitter ?>
                                </div>
                            </div>
                        </div>
                    </div>
